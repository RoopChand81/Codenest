...
import logo from '../../assets/Logo/MyLogo/Logo.png'
...
function Navbar() {
  ...
  return (
    ...
    >
                    ...
                    <div className="invisible absolute left-[50%] top-[50%] z-[1000] flex w-[200px] 
                     ...
                     >
                      ...
                      {loading ? (
                        ...
                        subLinks
                          .filter((link) => link?.courses?.length > -1)
                          .map((link, i) => (
                            <Link
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="absolute top-14 left-0 z-50 w-full bg-richblack-800 p-4 text-richblack-25 md:hidden">
              ...
              <li key={index}>
                {link.title === "Catalog" ? (
                  <div>
                    <div
                      onClick={() =>
                        setShowMobileCatalog((prev) => !prev)
                      }
                      className="flex cursor-pointer items-center gap-x-2 font-semibold"
                    >
                      <p>{link.title}</p>
                      {showMobileCatalog ? <BsChevronUp /> : <BsChevronDown />}
                    </div>
                    {showMobileCatalog && (
                      <div className="ml-3 mt-2 flex flex-col gap-2 text-sm text-richblack-100">
                        {loading ? (
                          <p>Loading...</p>
                        ) : subLinks.length > 0 ? (
                          subLinks
