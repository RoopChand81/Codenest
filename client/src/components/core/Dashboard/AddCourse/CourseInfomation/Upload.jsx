...
const Upload = ({name, label, register, errors, setValue}) => {
    ...
    const handelonchange = (e) => {
        const file = e.target.files[0];//take current store file
        setValue(name, e.target.files[0]);//set image in parent form
        //display or preview upoladed imaage
        if(file) {
    ...
    <div>
        {
            image ? (
                <div className="flex flex-col space-y-2">
                    <img src={image} alt="" className="h-full w-11/12 rounded-md object-cover"/>
                ...
                <div className="flex flex-col space-y-2">
                  ...
                  <div className="bg-richblack-700 flex min-h-[250px] cursor-pointer items-center justify-center rounded-md border-2 border-dotted border-richblack-500">
                    <div className="flex w-full flex-col items-center p-6" role="presentation"
                      tabIndex={0}
                    >
                      <input 
                        id={label} 
                        ...
                        tabIndex="-1" 
                        multiple=""  {...register(name, {required:true})} 
                        onChange={handelonchange} className="hidden" 
                      />
                        <div className="grid aspect-square w-14 place-items-center rounded-full bg-pure-greys-800">
                          {/* show the icon of file upload */}
                          <svg
                            stroke="currentColor"
                            fill="none"
                            strokeWidth="2"
